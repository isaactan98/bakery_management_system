...
function searchName(){
	...
	document.getElementById("searchPanel").action='viewOrder.php?page=1&method=1&searchValue='+x;
}
function searchHPN(){
	var x= document.getElementById("iconSearch").value;
	document.getElementById("searchPanel").action='viewOrder.php?page=1&method=2&searchValue='+x;
}
function searchType(){
	var x= document.getElementById("iconSearch").value;
	document.getElementById("searchPanel").action='viewOrder.php?page=1&method=3&searchValue='+x;
}
function searchAll(){
	document.getElementById("iconSearch").value='';
	document.getElementById("searchPanel").action='viewOrder.php?page=1&method=4&searchValue=';
}
</script>
<?php 
bakeryHeader();
...
<div class="container-fluid">
	<div class="row content">
		<div class="col-sm-2 sidenav">
			...
			<ul class="drop">
			<li class="active"><a class="one" href="viewOrder.php?page=1&method=4&searchValue="><?php spacing(7);?><i class="fas fa-caret-right" aria-hidden="true" style="margin-right:10px"></i>Order List</a></li>
		...
		<div class="col-sm-10">
			...
			if($method==1){
				...
				$sql = "SELECT COUNT(cust_ID) as noOfRecord FROM product_order WHERE cust_Name LIKE '".$searchKey."%' AND status='pending'"; 
			}
			else if($method==2){
				search($searchKey,$method);
				$sql = "SELECT COUNT(cust_ID) as noOfRecord FROM product_order WHERE cust_HPN LIKE '".$searchKey."%' AND status='pending'"; 
			}
			else if($method==3){
				search($searchKey,$method);
				$sql = "SELECT COUNT(cust_ID) as noOfRecord FROM product_order WHERE type LIKE '".$searchKey."%' AND status='pending'"; 
			}
			else {
				if($method==4){
					displayAll();
					$sql = "SELECT COUNT(cust_ID) as noOfRecord FROM product_order WHERE status='pending'"; 
				}
			}
			$recordPerPage=10;
			...
			if($thisPage==1)	// disable left arrow if first page
				...
				echo "<a href='viewOrder.php?page=".$LcurrentPage."&method=".$method."&searchValue=".$searchKey."'>&laquo;</a>";

			if($thisPage>$link+1){	// start to display first page with "..."
				echo '<a href="viewOrder.php?page=1&method='.$method.'&searchValue='.$searchKey.'">1</a>';
			...
			for ($i=$start; $i<=$end; $i++) {
				...
				if(isSet($_GET['page']) && $_GET['page']==$i)
					$active='class="active"';
				echo "<a $active href='viewOrder.php?page=".$start."&method=".$method."&searchValue=".$searchKey."'>".$start."</a>"; 
				$start++;
			} 
		
			if($end<$last){		// display last page with "..."
				echo '<a class="disableDot">...</a>';
				echo '<a href="viewOrder.php?page='.$last.'&method='.$method.'&searchValue='.$searchKey.'">'.$last.'</a>';
			}
		
			if($thisPage==$last)	// disable right arrow if last page
				echo "<a class='disable'>&raquo;</a>";	
			else
				echo "<a href='viewOrder.php?page=".$RcurrentPage."&method=".$method."&searchValue=".$searchKey."'>&raquo;</a>";
...
function displaySearchPanel(){
	...
	echo '<form id="searchPanel" method="post">';
	...
	echo '<input type="text" name="searchValue" placeholder="Enter the details" id="iconSearch" class="InputSearch" value='.$searchValue.'>';spacing(5);
	echo '<input type="submit" class="btn btn-primary" name="searchByName" value="By Name" onclick="searchName()">';spacing(1);
	echo '<input type="submit" class="btn btn-primary" name="searchByHPN" value="By HPN" onclick="searchHPN()">';spacing(1);
	echo '<input type="submit" class="btn btn-primary" name="searchByType" value="By Cake Type" onclick="searchType()">';spacing(1);
	echo '<input type="submit" class="btn btn-primary" name="displayAll" value="Display All" onclick="searchAll()">';
	echo '</fieldset>';
	echo '</div>';
	echo '</form>';
}

function search($searchKey,$method){
	$recordPerPage=10;
	$status=1;
	if(isSet($_GET['page'])){
		$page= $_GET['page'];
	...
	if($method==1){
		$sql="SELECT * from product_order WHERE cust_Name LIKE '".$searchKey."%' AND status='pending' ORDER BY cust_ID ASC LIMIT $start_from, $recordPerPage";
		$qry = mysqli_query($con,$sql);
		$totalOrder= searchOrderByName($searchKey,$status);
	}
	else if($method==2){
		$sql="SELECT * from product_order WHERE cust_HPN LIKE '".$searchKey."%' AND status='pending' ORDER BY cust_ID ASC LIMIT $start_from, $recordPerPage";
		$qry = mysqli_query($con,$sql);
		$totalOrder= searchOrderByHPN($searchKey,$status);
	}
	else{
		if($method==3){
			$sql="SELECT * from product_order WHERE type LIKE '".$searchKey."%' AND status='pending' ORDER BY cust_ID ASC LIMIT $start_from, $recordPerPage";
			$qry = mysqli_query($con,$sql);
			$totalOrder= searchOrderByType($searchKey,$status);
		}
	}
	...
	if($numOrder>0){
		if($numOrder>10)
			echo '<h4 style="margin-top:0;color:#337AB7;">Page '.$page.' of '.$numOrder.' order/s pending...</h4>';
		else
			echo '<h4 style="margin-top:0;color:#337AB7;">'.$numOrder.' order/s pending...</h4>';
		...
		while($row= mysqli_fetch_assoc($qry)){
			echo '<tr class="w3-hover-text-blue">';
				...
				echo '<td style="text-align:center;vertical-align:middle;">';
					echo "<form action='customerDetail.php?page=$page&method=$method&searchValue=$searchKey' method='post' style='display:inline'>";
					...
					echo '</form>';
					// delete-----------------------------
					echo "<button type='submit' onclick='confirmDelete(".$row['cust_ID'].",$method,\"$searchKey\")' title='Delete' name='deleteOrder' style='margin-right:2;padding:4;' class='btn btn-danger fa fa-trash-o' aria-hidden='true'>";
					// update order -----------------------------
					...
					echo '</form>';
					// update status -----------------------------
					echo "<button type='submit' onclick='confirmUpdate(".$row['cust_ID'].",$page,$method,\"$searchKey\")' title='one' name='updateStatus' style='margin-right:2;padding:4;' class='btn btn-success fa fa-check-square-o' aria-hidden='true'>";
				echo '</td>';
				//--------------------------------------------
			echo '</tr>';
			$count++;
		...
		while($row= mysqli_fetch_assoc($qry)){
			echo '<tr class="w3-hover-text-blue">';
				...
				echo '<td style="text-align:center;vertical-align:middle;">';
					...
					echo '</form>';
					// update status -----------------------------
					echo "<button type='submit' onclick='confirmUpdate(".$row['cust_ID'].",$page,$method,\"$searchValue\")' title='one' name='updateStatus' style='margin-right:2;padding:4;' class='btn btn-success fa fa-check-square-o' aria-hidden='true'>";
				echo '</td>';
				//--------------------------------------------
			echo '</tr>';
			$count++;
