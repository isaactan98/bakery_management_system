...
function searchName(){
	...
	document.getElementById("searchPanel").action='deliveryOrder.php?page=1&method=1&searchValue='+x;
}
function searchHPN(){
	var x= document.getElementById("iconSearch").value;
	document.getElementById("searchPanel").action='deliveryOrder.php?page=1&method=2&searchValue='+x;
}
function searchType(){
	var x= document.getElementById("iconSearch").value;
	document.getElementById("searchPanel").action='deliveryOrder.php?page=1&method=3&searchValue='+x;
}
function searchAll(){
	document.getElementById("iconSearch").value='';
	document.getElementById("searchPanel").action='deliveryOrder.php?page=1&method=4&searchValue=';
}
</script>
<?php 
bakeryHeader();
...
<div class="container-fluid">
	<div class="row content">
		<div class="col-sm-2 sidenav">
			...
			<li class="active"><a class="one" href="deliveryOrder.php?page=1&method=4&searchValue="><?php spacing(7);?><i class="fas fa-caret-right" aria-hidden="true" style="margin-right:10px"></i>Delivery List</a></li>
			</ul>
			</div>
		...
		<div class="col-sm-10">
			...
			if($method==1){
				...
				search($searchKey,$method);
				$sql = "SELECT COUNT(cust_ID) as noOfRecord FROM product_order WHERE cust_HPN LIKE '".$searchKey."%' AND status='deliver'"; 
			}
			else if($method==3){
				search($searchKey,$method);
				$sql = "SELECT COUNT(cust_ID) as noOfRecord FROM product_order WHERE type LIKE '".$searchKey."%' AND status='deliver'"; 
			}
			else {
				if($method==4){
					displayAll();
			...
			if($thisPage==1)	// disable left arrow if first page
				...
				echo "<a href='deliveryOrder.php?page=".$LcurrentPage."&method=".$method."&searchValue=".$searchKey."'>&laquo;</a>";

			if($thisPage>$link+1){	// start to display first page with "..."
				echo '<a href="deliveryOrder.php?page=1&method='.$method.'&searchValue='.$searchKey.'">1</a>';
			...
			for ($i=$start; $i<=$end; $i++) {
				...
				if(isSet($_GET['page']) && $_GET['page']==$i)
					$active='class="active"';
				echo "<a $active href='deliveryOrder.php?page=".$start."&method=".$method."&searchValue=".$searchKey."'>".$start."</a>"; 
				$start++;
			} 
		
			if($end<$last){		// display last page with "..."
				echo '<a class="disableDot">...</a>';
				echo '<a href="deliveryOrder.php?page='.$last.'&method='.$method.'&searchValue='.$searchKey.'">'.$last.'</a>';
			}
		
			if($thisPage==$last)	// disable right arrow if last page
				echo "<a class='disable'>&raquo;</a>";	
			else
				echo "<a href='deliveryOrder.php?page=".$RcurrentPage."&method=".$method."&searchValue=".$searchKey."'>&raquo;</a>";
...
function search($searchKey,$method){
	...
	if($method==1){
		...
		$sql="SELECT * from product_order WHERE cust_HPN LIKE '".$searchKey."%' AND status='deliver' ORDER BY cust_ID ASC LIMIT $start_from, $recordPerPage";
		$qry = mysqli_query($con,$sql);
		$totalOrder= searchOrderByHPN($searchKey,$status);
	}
	else{
		if($method==3){
			$sql="SELECT * from product_order WHERE type LIKE '".$searchKey."%' AND status='deliver' ORDER BY cust_ID ASC LIMIT $start_from, $recordPerPage";
			$qry = mysqli_query($con,$sql);
