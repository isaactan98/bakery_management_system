	...
	<div class="row content">
		...
		<div class="col-sm-10">
			...
			if($method==1){
				...
				$sql = "SELECT COUNT(cust_ID) as noOfRecord FROM product_order WHERE cust_Name LIKE '".$searchKey."%' AND status='done'"; 
			}
			else if($method==2){
				search($searchKey,$method);
				$sql = "SELECT COUNT(cust_ID) as noOfRecord FROM product_order WHERE cust_HPN LIKE '".$searchKey."%' AND status='done'"; 
			}
			else if($method==3){
				search($searchKey,$method);
				$sql = "SELECT COUNT(cust_ID) as noOfRecord FROM product_order WHERE type LIKE '".$searchKey."%' AND status='done'"; 
			}
			else {
				if($method==4){
					displayAll();
...
function search($searchKey,$method){
	...
	if($method==1){
		$sql="SELECT * from product_order WHERE cust_Name LIKE '".$searchKey."%' AND status='done' ORDER BY cust_ID ASC LIMIT $start_from, $recordPerPage";
		$qry = mysqli_query($con,$sql);
		$totalOrder= searchOrderByName($searchKey,$status);
	}
	else if($method==2){
		$sql="SELECT * from product_order WHERE cust_HPN LIKE '".$searchKey."%' AND status='done' ORDER BY cust_ID ASC LIMIT $start_from, $recordPerPage";
		$qry = mysqli_query($con,$sql);
		$totalOrder= searchOrderByHPN($searchKey,$status);
	}
	else{
		if($method==3){
			$sql="SELECT * from product_order WHERE type LIKE '".$searchKey."%' AND status='done' ORDER BY cust_ID ASC LIMIT $start_from, $recordPerPage";
			$qry = mysqli_query($con,$sql);
